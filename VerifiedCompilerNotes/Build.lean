...
partial def buildExercises
  ...
  let some mainDir := mainFileName.parent
    | throw <| IO.userError "Can't find directory of `VerifiedCompilerNotes/Build.lean`"
...
def htmlConfig : RenderConfig where
  emitTeX := false
  emitHtmlSingle := .no
  emitHtmlMulti := .immediately
  htmlDepth := 2

def pdfConfig : RenderConfig where
  emitTeX := true
  emitHtmlSingle := .no
  emitHtmlMulti := .no
  htmlDepth := 2
