...
partial def buildExercises (mode : Mode) (logError : String → IO Unit) (cfg : Config) (_state : TraverseState) (text : Part Manual) : IO Unit := do
  ...
  let some mainDir := mainFileName.parent
    | throw <| IO.userError "Can't find directory of `VerifiedCompilerNotesMain.lean`"
...
def main := manualMain (%doc VerifiedCompilerNotes) (extraSteps := [buildExercises]) (config := config)
